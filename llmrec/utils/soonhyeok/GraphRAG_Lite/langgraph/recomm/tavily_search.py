...
def tavily_for_recomm(llm, state:Lite_GraphState) : 
    prompt = ChatPromptTemplate.from_template(TAVILY_TEMPLATE)
    tools = TavilySearchResults(max_results=6, api_key = Env.tavily_api_key)
    ...
    state["selected_recommendations"] = recomm_list
