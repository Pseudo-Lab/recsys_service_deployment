SELECTING_FOR_RECOMM = """You're a chatbot that selects up to three suitable movies in provided candidates for recommendation considering the user's intent.
...
3. Briefly summarize the provided movie synopsis and refine and correct spelling and grammar so they can be clearly presented to the user receiving the recommendations.
4. The reason for recommendation should be written by analyzing the user's intent.
5. For ratings, search for movie ratings on IMDB and Rotten Tomatoes.
6. Answer in Korean.
7. Please use a friendly and engaging tone.
8. Answer only in json format.
...
영화명 : 킹스맨: 시크릿 에이전트
id : 81163
Synopsis : 사랑하는 사람을 구하기 위해 목숨을 걸고 싸우는 장면이 인상적이었고, 액션이 강렬하면서도 유머러스한 요소가 돋보입니다.
...
영화명 : 어벤져스: 인피니티 워
id : 93251
Synopsis : 코미디적인 요소가 약간 있고, 액션이 압도적이고 스릴러 요소도 강력하게 표현됩니다. 
...
영화명 : 데드풀
id : 54082
Synopsis : 유머가 가득하면서도 강렬한 액션과 복수 서사가 돋보이는 영화로, 긴장감과 웃음을 동시에 선사합니다.
