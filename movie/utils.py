...
def get_pop(mysql):
    ...
    daum_ratings = daum_ratings[
        daum_ratings["nickName"].map(
            lambda x: x not in ["휴면 사용자", "", "닉네임을 등록해 주세요", "닉네임"]
        )
    ]
    ...
    daum_movies["num_of_collected_ratings"] = daum_movies["movieId"].map(
        rating_num_dict
    )

    pop_movies_id = (
        daum_movies[daum_movies["num_of_collected_ratings"] > 100]
...
def add_past_rating(username, session_id, recomm_result: List[Dict]):
    ...
    if "star" in user_logs_df.columns:
        star_df = user_logs_df[user_logs_df["star"].notnull()].drop_duplicates(
            subset=["titleKo"], keep="last"
        )
        movie2rating = dict(
            zip(star_df["movieId"].astype(int), star_df["star"].astype(int))
        )
    else:
        movie2rating = {}
    for one_movie_d in recomm_result:
        one_movie_d["past_rating"] = (
            int(movie2rating.get(one_movie_d["movieid"], 0)) * 10
        )
...
def get_interacted_movie_dicts(user_logs_df, k=50):
    user_logs_df["timestamp"] = user_logs_df["timestamp"].astype(
        int
    )  # timestamp 열을 정수형으로 변환
    top_k_logs_df = user_logs_df.nlargest(k, "timestamp")

    if "star" in top_k_logs_df:
        top_k_logs_df["star"] = top_k_logs_df["star"].map(
            lambda x: float(int(x) / 2) if not pd.isna(x) else "click"
        )
    else:
        top_k_logs_df["star"] = "click"
    interacted_movie_d = top_k_logs_df[
        ["movieId", "titleKo", "timestamp", "star"]
...
def get_poster_urls(movie_ids):
    movies = DaumMovies.objects.filter(movieid__in=movie_ids).values(
        "movieid", "posterurl"
    )
