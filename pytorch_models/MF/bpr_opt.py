...
def retrain_model(interacted_items, num_recommendations=10, num_epochs=10, learning_rate=config.lr):
    ...
    # 학습 데이터 생성
    pos_samples_user = torch.tensor([new_num_user-1] * len(interacted_items)).to(device)
    ...
    for epoch in range(num_epochs):
      ...
      for pos_user, pos_item in zip(pos_samples_user, pos_samples_item) :
        ...
        neg_samples_user, neg_samples_item = pred_get_neg_samples(config.neg_samples, new_num_user-1, interacted_items)
    ...
    rec_items = predict_recommendations(model, new_num_user-1, interacted_items, num_recommendations)
    return rec_items
