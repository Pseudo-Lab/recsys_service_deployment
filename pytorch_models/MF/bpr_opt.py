...
all_items = [i for i in range(config.num_items)]
...
def retrain_model(interacted_items, num_recommendations=10, num_epochs=10, learning_rate=config.lr):
    # 먼저 모델 파라미터 불러와서 현재 모델의 user embedding 개수를 new_num_user 변수에 저장 -> MF 모델 클래스에 num_user 파라미터를 업데이트
    checkpoint = torch.load('pytorch_models/MF/MF_parameters.pt')
    checkpoint_dict = dict(OrderedDict(checkpoint))
    new_num_user = len(checkpoint_dict['user_emb.weight'])

    model = MF(new_num_user, config.num_items, config.k).to(device)
    model.load_state_dict(torch.load('pytorch_models/MF/MF_parameters.pt'))
    
    # # 임베딩 사이즈 출력
    # print("User Embedding Size:", model.user_emb.weight.shape)
    # print("Item Embedding Size:", model.item_emb.weight.shape)
    ...
    config.num_users += 1
    # print("config.num_users : ",config.num_users)
    ...
    # 학습 데이터 생성
    pos_samples_user = torch.tensor([config.num_users-1] * len(interacted_items)).to(device)
    ...
    for epoch in range(num_epochs):
      ...
      for pos_user, pos_item in zip(pos_samples_user, pos_samples_item) :
        ...
        neg_samples_user, neg_samples_item = pred_get_neg_samples(config.neg_samples, config.num_users-1, interacted_items)
    ...
    torch.save(model.state_dict(),'pytorch_models/MF/MF_parameters.pt')

    rec_items = predict_recommendations(model, config.num_users-1, interacted_items, num_recommendations)
    return rec_items

# 3. 재학습한 모델을 사용하여 새로운 아이템을 추천합니다.
def predict_recommendations(model, user_id, interacted_items, num_recommendations):
    # 사용자 ID를 Tensor로 변환
