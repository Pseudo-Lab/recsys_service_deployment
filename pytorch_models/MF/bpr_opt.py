...
config.num_users = 6039
def retrain_model(model, interacted_items, num_recommendations=10, num_epochs=10, learning_rate=config.lr):
    # 임베딩 사이즈 출력
    ...
    # 학습 데이터 생성
    pos_samples_user = torch.tensor([config.num_users] * len(interacted_items)).to(device)
    ...
    for epoch in range(num_epochs):
      ...
      for pos_user, pos_item in zip(pos_samples_user, pos_samples_item) :
        ...
        neg_samples_user, neg_samples_item = pred_get_neg_samples(config.neg_samples, config.num_users, interacted_items)
    ...
    rec_items = predict_recommendations(model, config.num_users, interacted_items, num_recommendations)
    print(model)
    return rec_items
